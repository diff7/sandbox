...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    if mask is not None:
        scaled_attention_logits += (
            mask * -1e9
        )  # make masked 1 values very small -1e9 but not zero
...
def dense_relu_dense(d_model, dff):
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, dff, rate=0.1):
        ...
        self.ffn = dense_relu_dense(d_model, dff)
    ...
    def call(self, x, training, mask):
        ...
        out1 = self.layernorm1(x + attn_output)  # (batch_size, input_seq_len, d_model)s

        # NOTE: DO we really need this part?
